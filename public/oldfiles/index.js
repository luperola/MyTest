...
//var inputFile = 'kitmondo.csv';
//var inputFile = 'tarasemi.csv';
...
app.post('/ebaysearch', (request, res) => {
    ...
    // Define global variable for the URL filter - DOES THIS GO HERE or literally right after "var filterarray = [ ..."
    var urlfilter = "";
...
const diskStorage = multer.diskStorage({
    ...
    filename: (req, file, call_back) => {
        //Prepend date to the filename or anything that makes the file unique so it won't be overwritten
        const m = Date.now();
        call_back(null, m + '_' + file.originalname);
    }
});
//Create Multer Instance
const upload = multer({ storage: diskStorage }).array("myImage", 10);
